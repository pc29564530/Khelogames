...
func (q *Queries) GetBadmintonMatchSetScore(ctx context.Context, matchPublicID uuid.UUID) ([]map[string]interface{}, error) {
	rows, err := q.db.QueryContext(ctx, getBadmintonMatchSetScore, matchPublicID)
	if err != nil {
		if err == sql.ErrNoRows {
			return nil, nil
		}
...
const getBadmintonMatchScore = `
    ...
    CASE
        ...
        THEN COUNT(*) FILTER (
            WHERE bs.home_score IS NOT NULL
			AND bs.away_score IS NOT NULL
            AND bs.home_score > bs.home_score
        )
