...
func (q *Queries) GetMatchModelByPublicId(ctx context.Context, publicID uuid.UUID) (*models.Match, error) {
	var match models.Match

	row := q.db.QueryRowContext(ctx, getMatchModelByPublicIdQuery, publicID)
	if err := row.Scan(
		&match.ID,
		&match.PublicID,
		&match.TournamentID,
		...
		if err == sql.ErrNoRows {
			log.Printf("Match model not found with public_id: %s", publicID)
			return nil, nil
		}
...
const getTrendingMatches = `
    SELECT 
        json_build_object(
            ...
            'homeScore', CASE 
                WHEN g.name = 'football' AND fs_home.id IS NOT NULL THEN 
                    json_build_object(
                        'id', fs_home.id,
                        'match_id', fs_home.match_id,
                        'team_id', fs_home.team_id,
    ...
    -- Football scores
    LEFT JOIN football_score fs_home ON fs_home.match_id = m.id AND fs_home.team_id = ht.id AND g.name = 'football' AND m.status_code='in_progress'
    LEFT JOIN football_score fs_away ON fs_away.match_id = m.id AND fs_away.team_id = at.id AND g.name = 'football' AND m.status_code='in_progress'
    ...
    LEFT JOIN LATERAL (
        ...
        FROM cricket_score cs
        WHERE cs.match_id = m.id AND cs.team_id = ht.id AND m.status_code='in_progress'
    ) AS cricket_home_scores ON true
...
func (q *Queries) GetTrendingMatches(ctx context.Context, game int64) ([]map[string]interface{}, error) {
	rows, err := q.db.QueryContext(ctx, getTrendingMatches, game)
	if err != nil {
		log.Printf("Failed to query: %v", err)
		return nil, err
	}
	...
	var trendingMatches []map[string]interface{}
	for rows.Next() {
		var trendingMatch map[string]interface{}
		var jsonByte []byte
		err := rows.Scan(&jsonByte)
		if err != nil {
			log.Printf("Failed to scan row: %v", err)
		...
		err = json.Unmarshal(jsonByte, &trendingMatch)
		if err != nil {
			log.Printf("Failed to unmarshal: %v", err)
			return nil, err
		}
		trendingMatches = append(trendingMatches, trendingMatch)
	}
