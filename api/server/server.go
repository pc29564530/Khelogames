...
func NewServer(config util.Config,
	...
	authRouter := router.Group("/api").Use(authMiddleware(server.tokenMaker))
	{
		...
		authRouter.GET("/getUserByCommunity/:community_public_id", handlersServer.GetUserByCommunityFunc)
		...
		authRouter.GET("/get_communities_member/:community_public_id", handlersServer.GetCommunitiesMemberFunc)
