...
func NewServer(config util.Config,
	...
	authRouter := router.Group("/api").Use(authMiddleware(server.tokenMaker))
	{
		...
		authRouter.POST("/create_follow/:following_owner", handlersServer.CreateUserConnectionsFunc)
		...
		authRouter.PUT("/updateThreadCommentCount", handlersServer.UpdateThreadCommentCountFunc)
	}
