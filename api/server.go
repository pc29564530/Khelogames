...
func NewServer(config util.Config, store *db.Store) (*Server, error) {
	...
	public := router.Group("/auth")
	{
		...
		public.POST("/signup", server.createSignup)
		public.POST("/users", server.createUser)
		public.POST("/login", server.createLogin)
		public.DELETE("/removeSession/:username", server.deleteSession)
		public.POST("/tokens/renew_access", server.renewAccessToken)
		public.GET("/user/:username", server.getUsers)
		public.GET("/getProfile/:owner", server.getProfile)
	}
	...
	authRouter := router.Group("/api").Use(authMiddleware(server.tokenMaker))
	{
		...
		authRouter.GET("/getPlayerProfile", server.getPlayerProfile)
		authRouter.GET("/getAllPlayerProfile", server.getAllPlayerProfile)
		authRouter.POST("/addPlayerProfile", server.addPlayerProfile)
		authRouter.PUT("/updatePlayerProfileAvatarUrl", server.updatePlayerProfileAvatar)
	...
	sportRouter.GET("/getTournament/:tournament_id", server.getTournament)
	sportRouter.POST("/addFootballMatchScore", server.addFootballMatchScore)
	sportRouter.GET("/getFootballMatchScore", server.getFootballMatchScore)
	sportRouter.PUT("/updateFootballMatchScore", server.updateFootballMatchScore)
	sportRouter.POST("/addFootballGoalByPlayer", server.addFootballGoalByPlayer)
	sportRouter.GET("/getClub/:id", server.getClub)
	sportRouter.GET("/getClubs", server.getClubs)
	sportRouter.GET("/getClubMember", server.getClubMember)
	sportRouter.GET("/getAllTournamentMatch", server.getAllTournamentMatch)

	sportRouter.POST("/addCricketMatchScore", server.addCricketMatchScore)
	sportRouter.GET("/getCricketMatchScore", server.getCricketMatchScore)
	sportRouter.PUT("/updateCricketMatchRunsScore", server.updateCricketMatchRunsScore)
	sportRouter.PUT("/updateCricketMatchWicket", server.updateCricketMatchWicket)
	sportRouter.PUT("/updateCricketMatchExtras", server.updateCricketMatchExtras)
	sportRouter.PUT("/updateCricketMatchInnings", server.updateCricketMatchInnings)
	sportRouter.POST("/addCricketMatchToss", server.addCricketMatchToss)
	sportRouter.GET("/getCricketMatchToss", server.getCricketMatchToss)
	sportRouter.POST("/addCricketTeamPlayerScore", server.addCricketTeamPlayerScore)
	sportRouter.GET("/getCricketTeamPlayerScore", server.getCricketTeamPlayerScore)
	sportRouter.GET("/getCricketPlayerScore", server.getCricketPlayerScore)
	sportRouter.PUT("/updateCricketMatchScoreBatting", server.updateCricketMatchScoreBatting)
	sportRouter.PUT("/updateCricketMatchScoreBowling", server.updateCricketMatchScoreBowling)

	sportRouter.GET("/getClubPlayedTournaments", server.getClubPlayedTournaments)
	sportRouter.GET("/getClubPlayedTournament", server.getClubPlayedTournament)
	sportRouter.GET("/getTournamentsByClub", server.getTournamentsByClub)
	sportRouter.GET("/getMatchByClubName", server.getMatchByClubName)
	sportRouter.PUT("/updateTournamentDate", server.updateTournamentDate)

	sportRouter.POST("/createTournamentStanding", server.createTournamentStanding)
	sportRouter.POST("/createTournamentGroup", server.createTournamentGroup)
