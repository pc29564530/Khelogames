...
const addFootballLineUp = `-- name: AddFootballLineUp :one
...
func (q *Queries) AddFootballLineUp(ctx context.Context, arg AddFootballLineUpParams) (FootballLineup, error) {
...
const getFootballLineUp = `-- name: GetFootballLineUp :many
...
func (q *Queries) GetFootballLineUp(ctx context.Context, arg GetFootballLineUpParams) ([]FootballLineup, error) {
	rows, err := q.db.QueryContext(ctx, getFootballLineUp, arg.MatchID, arg.TeamID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []FootballLineup
	for rows.Next() {
		var i FootballLineup
		if err := rows.Scan(
			&i.ID,
...
const updateFootballSubsAndLineUp = `-- name: UpdateFootballSubsAndLineUp :one
WITH 
    sub AS (
